...
class IndexController extends Zend_Controller_Action {
    ...
    public function dashboardAction()
    {
        if (isset($this->session->auth)) {
            ...
            $user = $objectManager->getObject('user', $this->session->userId);
            $dashboard = new Application_Model_DashboardManager($user);
            $this->view->ownCurrentMonth = $dashboard->getOwnFormsCurrentMonth();
            $this->view->ownPrevMonth = $dashboard->getOwnFormsPrevMonth();
            $this->view->ownCurrentYear = $dashboard->getOwnFormsCurrentYear();
        } else {
            $this->_forward('index');
        }
    }
    ...
    public function newDomainAction()
    {
        ...
        if (!$this->form->isValid($this->_request->getParams())) {
            ...
            // Create new domain
            $domain = new Application_Model_Domain(array(
                'domainName' => $this->getRequest()->getParam('companyName') . ' domain',
                'hash'       => md5(time())));
            $domainId = $objectManager->saveObject($domain);

            // Update $objectManager with new domainId
            $objectManager->setDomainId($domainId);
            $node = new Application_Model_Node(array(
                'nodeName'     => $this->getRequest()->getParam('companyName'),
                'domainId'     => $domainId,
                'parentNodeId' => -1));
            $nodeId = $objectManager->saveObject($node);
            $position = new Application_Model_Position(array(
            ...
            $userGroup = new Application_Model_UserGroup(array(
                'userId'        => $userId,
                'domainId'      => $domainId,
                'role'          => 'admin',
                'userGroupName' => 'admin'));
